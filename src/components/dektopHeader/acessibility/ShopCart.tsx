...
export const ShopCart = () => {
  ...
  return (
    <div onClick={() => setOpenShop(!openShop)} className="cursor-pointer">
      <svg
      ...
      <div  className="absolute top-7 left-[305px] flex items-center justify-center pt-[3px] w-5 h-5 text-xs font-Dana text-zinc-700 bg-white border-2 border-orange-200 rounded-full">
        <span  className={shopProduct.length ? "animate-ping absolute top-0 right-0 w-4 h-4 rounded-full bg-orange-200 opacity-75" : "hidden"}></span>
        {shopProduct.length}
      </div>
