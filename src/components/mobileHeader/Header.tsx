...
export const Header = () => {
  ...
  return (
    <div className="container relative z-50 h-16 min-w-full w-full py-3 px-4 bg-white flex items-center justify-between dark:bg-zinc-700 lg:hidden">
      ...
      <div className="absolute top-3 left-8 flex items-center justify-center pt-[3px] w-4 h-4 text-xs font-Dana text-zinc-700 bg-orange-200 border-2 border-orange-200 rounded-full dark:text-zinc-700 dark:bg-white">{shopProduct.length}</div>
      <IconSvg />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
      ...
      {menuOpen && <MobileMenu isOpen={setisMenuOpen} isMenuOpen={isMenuOpen}/>}
      {shopCartOpen && <MobileShopCart isOpen={setisShopCartOpen} isShopOpen={isShopOpen}/>}
    </div>
  );
};
